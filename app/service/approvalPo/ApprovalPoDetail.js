...
function SubmitApprovalPurchaseOrder() {
  var form = $("#formApprovalPo");
  var dto = {
    poNumber: form.find("label[name='poNumber']").text(),
  ...
  $.ajax({
    type: "POST",
    url: BASE_URL + "ApprovalPo/SubmitApprovalPo",
    ...
    success: function (response) {
      ...
      if (response.status == 201) {
        Swal.fire({
          ...
          window.location = BASE_URL + "ApprovalPo";
        });
      }
    },
    error: function (err) {
